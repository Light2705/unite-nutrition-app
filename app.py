...
# ==========================================
    ...
    if st.session_state.admin and menu == "Gestión de Clientes":
        ...
        for alum in alumnos_check:
            ...
            if reg_hoy == 0 and hoy_dt.hour >= 16:
    ...
    # --- Sección: Mi Diario ---
        ...
        for tiempo in tiempos:
            ...
            with st.expander(f"➕ Agregar a {tiempo}"):
                ...
                query_alim = "SELECT * FROM master_food" if c_sel == "Todas" else "SELECT * FROM master_food WHERE category=?"
                df_alims = pd.read_sql(query_alim, conn, params=((c_sel,) if c_sel != "Todas" else None))
                ...
                if a_sel and a_sel != "➕ OTRO":
                    ...
                    fac = gramos / 100
                    p_p, c_p, g_p, k_p = f_data['p100']*fac, f_data['c100']*fac, f_data['g100']*fac, f_data['k100']*fac
                    st.markdown(f"📊 **Previo:** {p_p:.1f}P | {c_p:.1f}C | {g_p:.1f}G | **{int(k_p)} Kcal**")
